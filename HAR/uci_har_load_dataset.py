...
Original file is located at
    https://colab.research.google.com/drive/1aiLi5PrPBDBnkZONqxZ8u3VLEAFYvR7_
...
Source for .py version (use File > Download .py)
...
def uci_har_load_dataset(
    ...
    incl_val_group = False, # must be False for UCI HAR - already split train/test
    one_hot_encode = True
    ):
    """processes UCI HAR zip file into numpy arrays, returns x_train, y_train, x_test, y_test"""
    if (incl_val_group):
        print("UCI HAR is pre-split into train/test, incl_val_group = TRUE not supported")
